...
Write-Host "  自動啟動應用程式並執行測試" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# 1. 停止現有進程
Write-Host "[1/5] 停止現有 Java 進程..." -ForegroundColor Yellow
...
# 2. 檢查服務
Write-Host ""
Write-Host "[2/5] 檢查服務狀態..." -ForegroundColor Yellow
...
try {
    ...
    if ($redisTest -match "PONG") {
        ...
        Write-Host "  ✗ Redis 異常" -ForegroundColor Red
    }
} catch {
    Write-Host "  ✗ Redis 無法連接" -ForegroundColor Red
}

# 檢查 RabbitMQ
try {
    $response = Invoke-WebRequest -Uri "http://localhost:15672" -TimeoutSec 3 -UseBasicParsing -ErrorAction SilentlyContinue
    if ($response.StatusCode -eq 200) {
        Write-Host "  ✓ RabbitMQ 正常" -ForegroundColor Green
    }
} catch {
    Write-Host "  ⚠ RabbitMQ 可能未運行" -ForegroundColor Yellow
}

# 3. 編譯專案
Write-Host ""
Write-Host "[3/5] 編譯專案..." -ForegroundColor Yellow
...
# 4. 啟動應用程式（在新窗口）
Write-Host ""
Write-Host "[4/5] 啟動應用程式..." -ForegroundColor Yellow
...
# 5. 等待應用程式啟動
Write-Host ""
Write-Host "[5/5] 等待應用程式啟動..." -ForegroundColor Yellow
...
if (-not $started) {
    ...
    Write-Host "  如果應用程式已啟動，可以手動執行測試腳本" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  執行測試: .\WebSocket測試腳本.ps1" -ForegroundColor Cyan
    exit 0
}
